...
export default function EditVehicle() {
  ...
  const [newImages, setNewImages] = useState([]); // { uri, fileName }
  const MAX_IMAGES = 5;
  ...
  const fetchVehicle = async () => {
    try {
      ...
      setForm({
        ...
        brand: v.brand || "",
        model: v.model || "",
        manufacturedYear: v.manufacturedYear ? String(v.manufacturedYear) : "",
        registeredYear: v.registeredYear ? String(v.registeredYear) : "",
        maintenanceLevel: v.maintenanceLevel || "",
        maintenancePeriod: v.maintenancePeriod || "",
        price: v.price ? String(v.price) : "",
        transmission: v.transmission || "",
        fuelType: v.fuelType || "",
        engineCapacity: v.engineCapacity ? String(v.engineCapacity) : "",
  ...
  const pickImages = async () => {
    ...
    if (!result.canceled) {
      const addedImgs = result.assets.map((a) => ({ uri: a.uri, fileName: a.fileName || `photo_${Date.now()}.jpg` }));
      setNewImages((prev) => [...prev, ...addedImgs].slice(0, MAX_IMAGES - existingImages.length));
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Object.keys(form).forEach((key) => {
        if (form[key]) data.append(key, String(form[key]));
      });
  ...
  if (fetching) {
    return (
      <View style={{ flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "#f3f4f6" }}>
        ...
        <Text style={{ marginTop: 10, color: "#6b7280" }}>Loading vehicle details...</Text>
      </View>
    );
  }
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        ...
        >
          ...
          <View style={s.hero}>
            ...
            <View style={s.blob1} />
            <View style={s.blob2} />
            <Text style={s.heroTitle}>Edit Vehicle ✏️</Text>
            <Text style={s.heroSub}>Update your listing details</Text>
          </View>
          ...
          <View style={s.formBody}>
            ...
            <View style={s.card}>
              ...
              <Field label="Price (Rs.) *" value={String(form.price)} onChangeText={(v) => setField("price", v)} keyboardType="numeric" required />
              ...
              <View style={s.row}>
                <View style={{ flex: 1 }}>
                  <Field label="Engine Capacity (cc)" value={String(form.engineCapacity)} onChangeText={(v) => setField("engineCapacity", v)} keyboardType="numeric" />
                </View>
                <View style={{ flex: 1 }}>
                  <Field label="Mileage (km)" value={String(form.mileage)} onChangeText={(v) => setField("mileage", v)} keyboardType="numeric" />
                </View>
              </View>
            </View>
            ...
            <View style={s.card}>
              ...
              {(existingImages.length > 0 || newImages.length > 0) && (
                <View style={s.imageGrid}>
                  ...
                  {existingImages.map((img, index) => (
                    <View key={`ex-${index}`} style={s.imageItem}>
                      <Image source={{ uri: buildImageUrl(img) }} style={s.imageThumbnail} />
                      ...
                      <TouchableOpacity style={s.deleteBtn} onPress={() => removeExistingImage(img)}>
                        <Text style={s.deleteBtnText}>✕</Text>
                      </TouchableOpacity>

                      <View style={s.moveRow}>
                        <TouchableOpacity
                          onPress={() => moveExistingImage(index, -1)}
                          disabled={index === 0}
                          style={[s.moveBtn, index === 0 && s.moveBtnDisabled]}
                        >
                          <Text style={s.moveBtnText}>←</Text>
                        </TouchableOpacity>
                        <Text style={s.imageIndexText}>Old</Text>
                        <TouchableOpacity
                          onPress={() => moveExistingImage(index, 1)}
                          disabled={index === existingImages.length - 1}
                          style={[s.moveBtn, index === existingImages.length - 1 && s.moveBtnDisabled]}
                        >
                          <Text style={s.moveBtnText}>→</Text>
                        </TouchableOpacity>
                      </View>
                  ...
                  {newImages.map((img, index) => {
                    ...
                    return (
                      <View key={`new-${index}`} style={s.imageItem}>
                        ...
                        <TouchableOpacity style={s.deleteBtn} onPress={() => removeNewImage(index)}>
                          <Text style={s.deleteBtnText}>✕</Text>
                        </TouchableOpacity>
                        
                        <View style={s.moveRow}>
                           <Text style={[s.imageIndexText, { flex: 1, textAlign: "center" }]}>New</Text>
                        </View>
                      </View>
                    );
                  })}
              ...
              {/* Pick button */}
              <TouchableOpacity
                style={[s.pickImageBtn, (existingImages.length + newImages.length) >= MAX_IMAGES && s.pickImageBtnDisabled]}
                onPress={pickImages}
              ...
              >
                ...
                <Text style={s.pickImageText}>
                  {(existingImages.length + newImages.length) >= MAX_IMAGES
                    ...
                    : `Add Photos (${existingImages.length + newImages.length}/${MAX_IMAGES})`}
                </Text>
              </TouchableOpacity>
            </View>
            ...
            >
              {loading ? (
                <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                  ...
                  <Text style={s.submitBtnText}>Updating...</Text>
                </View>
              ) : (
                <Text style={s.submitBtnText}>Update Vehicle 🚀</Text>
              )}
            </TouchableOpacity>
